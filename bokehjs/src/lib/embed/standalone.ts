...
export async function add_document_standalone(document: Document, element: HTMLElement,
  ...
  async function render_model(model: HasProps, count: number): Promise<View> {
    let root_el: HTMLElement
    if ((count < root_ids.length) && (root_ids[count] in roots))
      root_el = roots[root_ids[count]]
  ...
  document.on_change((event: DocumentChangedEvent): void => {
    if (event instanceof RootAddedEvent) {
      const count = document.roots().indexOf((event.model as any))
      render_model(event.model, count)
